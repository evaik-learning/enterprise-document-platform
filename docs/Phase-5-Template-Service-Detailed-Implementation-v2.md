...
# 13A. Placeholder Management — First-Class Capability

Placeholder Management is a core responsibility of the Template Service.

The service supports both automatic Placeholder Discovery from uploaded `.docx` templates and full user-managed Placeholder Definition CRUD.
...
## 13A.1 Responsibilities

Placeholder Management owns:

- Placeholder definition creation
- Placeholder definition retrieval
- Placeholder definition update
- Placeholder definition deletion
- Placeholder metadata
...
- Placeholder/document consistency validation
- Tenant/organization isolation
- Authorization
- Placeholder audit events

It does not own final document value substitution. That responsibility belongs to the Document Service in Phase 6.

## 13A.2 Placeholder Definition Model
...
## 13A.3 Full Placeholder CRUD API

```http
POST   /api/v1/templates/{templateId}/versions/{versionId}/placeholders
GET    /api/v1/templates/{templateId}/versions/{versionId}/placeholders
GET    /api/v1/templates/{templateId}/versions/{versionId}/placeholders/{placeholderId}
PUT    /api/v1/templates/{templateId}/versions/{versionId}/placeholders/{placeholderId}
DELETE /api/v1/templates/{templateId}/versions/{versionId}/placeholders/{placeholderId}
```
...
```json
{
  ...
  "defaultValue": null,
  "maxLength": 200,
  "exampleValue": "ABC Technologies Pvt Ltd"
}
```

The placeholder `Name` should normally be immutable after creation because the actual Word document may contain `{{CustomerName}}`.

Deletion should be rejected with `409 Conflict` if the placeholder is still referenced by the document, unless a future controlled operation explicitly removes it from the document.

## 13A.4 Placeholder Discovery API

```http
POST /api/v1/templates/{templateId}/versions/{versionId}/placeholders/discover
```
...
Compare with existing definitions
    |
    +---- New placeholders ----> Create
    |
    +---- Existing ------------> Preserve user metadata
    |
    +---- Missing from DOCX ----> Mark inconsistent
    |
    v
Return discovery result
```

Discovery must not overwrite user-managed metadata such as `DataType`, `Description`, `IsRequired`, `DefaultValue`, `ValidationRule`, or `Format`.
...
## 13A.5 User-Managed Placeholder Metadata

The following metadata is configurable by authorized users:

| Property | User Managed | Required |
|---|---:|---:|
| Name | Limited | Yes |
| DisplayName | Yes | No |
| DataType | Yes | Yes |
...
| ExampleValue | Yes | No |
| Status | System controlled | Yes |
| SourceType | System generated | Yes |

## 13A.6 Validation and Consistency

Validate:

1. Placeholder syntax.
2. Placeholder definition metadata.
3. Data-type-specific rules.
4. Duplicate names.
5. Document/definition consistency.

Possible states:

```text
Document placeholder exists + definition exists
    = Valid

Document placeholder exists + definition missing
    = Missing Definition

Definition exists + document placeholder missing
    = Orphaned Definition
```

A template version should not become `Active` while unresolved placeholder errors exist.

## 13A.7 Placeholder Permissions

```text
TemplatePlaceholder.Read
TemplatePlaceholder.Create
TemplatePlaceholder.Update
TemplatePlaceholder.Delete
TemplatePlaceholder.Discover
TemplatePlaceholder.Validate
```
...
TemplateManager
    -> Full placeholder management

TemplateContributor
    -> Read/Create/Update/Discover

Viewer
    -> Read only
```

## 13A.8 Placeholder Audit Events
...
PlaceholderUpdated
PlaceholderDeleted
PlaceholderDiscovered
PlaceholderValidated
PlaceholderActivated
PlaceholderDeactivated
```
...
Do not store sensitive placeholder values in audit logs.

## 13A.9 Placeholder Application Structure
...
## 13A.10 Placeholder Infrastructure
...
└── Parsing/
    ├── ITemplateParser.cs
    ├── OpenXmlTemplateParser.cs
    ├── PlaceholderExtractor.cs
    ├── PlaceholderNormalizer.cs
    ├── PlaceholderSyntaxValidator.cs
    └── TemplateConsistencyValidator.cs
```

The parser must correctly handle placeholders split across Word runs:
...
## 13A.11 Phase 6 Integration

Phase 6 Document Service consumes Placeholder Definitions as the input contract for document generation.
...
The Document Service must consume the Template Service placeholder contract rather than create a separate placeholder-definition model.
...
- [ ] User-managed placeholder metadata implemented.
- [ ] Placeholder lifecycle implemented.
- [ ] Placeholder data type and validation rules implemented.
...
- [ ] Placeholder validation endpoint.
...
- [ ] Concurrency tests.
- [ ] Placeholder CRUD tests.
- [ ] Placeholder discovery tests.
- [ ] Cross-run DOCX placeholder tests.
- [ ] Placeholder/document consistency tests.
...
5. DOCX parser
6. Placeholder discovery engine
7. Full Placeholder Definition Management (CRUD)
8. Placeholder validation engine
8. REST APIs
